...
import { summarizePeriod, getDatesForPeriod, weeklyProfitData, monthlyProfitData, yearlyProfitData, platformRevenueShare, expenseBreakdown } from "../calc";
...
export function AnalyticsTab({ data }: Props) {
  ...
  const chartData = period === "weekly" ? weeklyProfitData(data) : period === "monthly" ? monthlyProfitData(data) : period === "yearly" ? yearlyProfitData(data) : weeklyProfitData(data);
  const chartKey = period === "weekly" ? "day" : period === "monthly" ? "month" : "year";
  const chartTitle = period === "weekly" ? "Haftalık Kâr Grafiği" : period === "monthly" ? "Aylık Kâr Grafiği" : period === "yearly" ? "Yıllık Kâr Grafiği" : "Haftalık Kâr Grafiği";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card">
        <h3 className="mb-3 font-semibold text-slate-900 dark:text-slate-100">{chartTitle}</h3>
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={chartData}>
            <XAxis dataKey={chartKey} tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={false} tickLine={false} />

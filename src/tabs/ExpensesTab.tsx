...
import { Fuel as FuelIcon, Wrench, Shirt, OctagonAlert as AlertOctagon, Utensils, Calculator, MoveHorizontal as MoreHorizontal, Trash2, Plus, TrendingDown, Check, Loader as Loader2, CircleAlert as AlertCircle, Calendar } from "lucide-react";
...
export function ExpensesTab({ data, updateOzet, user, selectedDate, onDateChange }: Props) {
  ...
  const [digerAdi, setDigerAdi] = useState("");
    ...
    for (const [cat, val] of Object.entries(ozet.gider_detaylari)) {
      if (val && val > 0) {
        selectedExpenses.push({
          ...
          amount: val,
          title: cat === "diger"
            ? (ozet.notlar_detay?.diger_isim || EXPENSE_CATEGORIES[cat as ExpenseCategory]?.label || cat)
            : (EXPENSE_CATEGORIES[cat as ExpenseCategory]?.label ?? cat),
  ...
  const addExpense = () => {
    ...
    updateOzet(selectedDate, (o) => {
      ...
      if (cat === "diger") {
        if (isim) newNotlarDetay["diger_isim"] = isim;
        else delete newNotlarDetay["diger_isim"];
      }
      return { ...o, gider_detaylari: newDetay, notlar_detay: newNotlarDetay };
    });
    setAmount(""); setDigerAdi(""); setArizaAdi(""); setModalOpen(false);
  };
  ...
  const removeExpense = (dateStr: string, cat: ExpenseCategory) => {
    updateOzet(dateStr, (o) => {
      ...
      delete newDetay[cat];
      const newNotlarDetay = { ...o.notlar_detay };
      if (cat === "diger") delete newNotlarDetay.diger_isim;
      return { ...o, gider_detaylari: newDetay, notlar_detay: newNotlarDetay };
    });
  };
        ...
        <div className="space-y-3">
          {category === "diger" && (
            <div className="fade-in">
              <label className="label">İsim <span className="badge-optional">Opsiyonel</span></label>
              <input
                type="text"
                className="input"
                placeholder="Örn: Telefon, otopark, kargo..."
                value={digerAdi}
                onChange={(e) => setDigerAdi(e.target.value)}
              />
            </div>
          )}

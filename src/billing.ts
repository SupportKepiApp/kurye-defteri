...
export async function purchaseSubscription(plan: PlanType): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    return { success: false, error: "Satın alma başarısız oldu. Lütfen tekrar deneyin." };
  } catch (e: any) {
    ...
    return { success: false, error: "Satın alma sırasında bir hata oluştu. Lütfen tekrar deneyin." };
  }
}

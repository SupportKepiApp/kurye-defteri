...
import { TRIAL_DAYS, SUBSCRIPTION_DURATIONS, DEV_EMAIL } from "./types";
...
export function getSubscriptionDaysRemaining(settings: Settings): number {
  ...
  if (settings.subscriptionExpiresAt) {
    const expiry = new Date(settings.subscriptionExpiresAt);
    const now = new Date();
...
export function getDaysRemaining(settings: Settings, email?: string | null): number {
  if (isDevUser(email)) return 9999;
...
export async function fetchSubscriptionFromSupabase(userId: string): Promise<{
...
export function computeExpiresAt(plan: SubscriptionPlan, fromDate?: Date): string {
  const base = fromDate ?? new Date();
  const expiry = new Date(base);
  expiry.setDate(expiry.getDate() + SUBSCRIPTION_DURATIONS[plan]);

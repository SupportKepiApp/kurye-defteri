...
import { loadData, saveData, todayStr, getTrialDaysRemaining, isSubscriptionExpired, isAccessLocked, getDaysRemaining, getSubscriptionDaysRemaining, fetchAllOzetlerFromSupabase, upsertOzetToSupabase, deleteOzetFromSupabase, createEmptyOzet, DEFAULT_DATA, fetchSubscriptionFromSupabase, upsertSubscriptionToSupabase, computeExpiresAt, isDevUser } from "./storage";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      if (u) {
        ...
        fetchSubscriptionFromSupabase(u.id)
          .then((sub) => {
            if (!mounted || !sub || !sub.expires_at) return;
            const expiry = new Date(sub.expires_at);
            const now = new Date();
            if (expiry > now) {
              const plan: SubscriptionPlan = sub.plan_type === "yearly" || sub.plan_type === "365_gun" ? "yearly" : "monthly";
              setData((prev) => ({
                ...prev,
                settings: {
                  ...prev.settings,
                  subscriptionState: "active",
                  subscriptionStartDate: prev.settings.subscriptionStartDate ?? todayStr(),
                  subscriptionPlan: plan,
                  subscriptionExpiresAt: sub.expires_at,
                },
              }));
            } else if (sub.expires_at && expiry <= now) {
              setData((prev) => ({
                ...prev,
                settings: { ...prev.settings, subscriptionState: "expired" },
              }));
            }
          })
          .catch(() => {});
  ...
  const subDaysLeft = isDev ? 9999 : getSubscriptionDaysRemaining(data.settings);
  const expired = !isDev && isAccessLocked(data.settings);
  const daysLeft = getDaysRemaining(data.settings, user?.email);
  ...
  const handleSelectPlan = async (plan: SubscriptionPlan) => {
    if (isBillingAvailable()) {
      ...
      if (result.success) {
        ...
        updateData((d) => ({ ...d, settings: { ...d.settings, subscriptionState: "active", subscriptionStartDate: todayStr(), subscriptionPlan: plan, subscriptionExpiresAt: expiresAt } }));
        if (user) {
          upsertSubscriptionToSupabase(user.id, user.email ?? "", plan, expiresAt).catch(() => {});
        }
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col bg-slate-100 dark:bg-slate-950">
      ...
      <header className="border-b border-slate-200 bg-slate-100 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            {isDev ? (
              <button onClick={() => setPaywallOpen(true)} className="tap-target inline-flex items-center gap-1 rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700 dark:bg-green-900/30 dark:text-green-400">
                <Clock size={12} /> Premium
              </button>
      ...
      <Paywall open={paywallOpen} onClose={() => { if (!expired) setPaywallOpen(false); }} trialDaysLeft={trialDaysLeft} subDaysLeft={subDaysLeft} subscriptionState={data.settings.subscriptionState} onSelectPlan={handleSelectPlan} onRestore={handleRestorePurchase} loading={billingLoading} forceOpen={expired} isDev={isDev} />
    </div>
  );
}

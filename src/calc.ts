...
export function getDatesForPeriod(period: "daily" | "weekly" | "monthly" | "yearly"): string[] {
  ...
  if (period === "daily") { dates.push(today.toISOString().slice(0, 10)); }
    ...
    for (let d = 1; d <= 366; d++) { const date = new Date(y, 0, d); if (date.getFullYear() !== y || date > today) break; dates.push(date.toISOString().slice(0, 10)); }
  }
  return dates;
}
...
export function monthlyProfitData(data: AppData): { month: string; net: number }[] {
  const today = new Date();
  const y = today.getFullYear();
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (let d = 1; d <= lastDay; d++) {
      ...
      const ozet = data.ozetler[dateStr];
      if (ozet) net += netKazanc(ozet);
    }
    result.push({ month: MONTH_LABELS[m], net: Math.round(net) });
  }
  return result;
}

export function yearlyProfitData(data: AppData): { year: string; net: number }[] {
  const today = new Date();
  const currentYear = today.getFullYear();
  const result: { year: string; net: number }[] = [];
  for (let y = currentYear - 3; y <= currentYear + 1; y++) {
    let net = 0;
    for (const dateStr of Object.keys(data.ozetler)) {
      if (dateStr.startsWith(String(y))) {
        net += netKazanc(data.ozetler[dateStr]);
      }
    }
    result.push({ year: String(y), net: Math.round(net) });
  }
  return result;
}
